// WAF用のプロバイダー (CloudFrontはus-east-1が必要)
// CloudFront用のWAF WebACLは必ずus-east-1リージョンに作成する必要がある
provider "aws" {
  alias   = "us_east_1"
  region  = "us-east-1"
...
// CloudFront経由のみのアクセスとするため、完全にプライベートに設定
resource "aws_s3_bucket_public_access_block" "website_bucket" {
  // 対象のS3バケットIDを指定
  bucket = aws_s3_bucket.website_bucket.id

  // CloudFrontのOAC (Origin Access Control) を使用するため、すべてブロック
...
// OAC (Origin Access Control) を使用してCloudFrontからのみアクセス可能にする
...
resource "aws_s3_object" "upload_file" {
  ...
  // CloudFront経由のみアクセス可能なため、バケットポリシーで制御
  // acl = "private"
}
...
resource "aws_s3_object" "upload_images" {
  ...
  // 注意: このリソースはプライベートとして扱われ、CloudFront経由でのみアクセス可能
}
